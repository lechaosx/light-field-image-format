...
public:
  explicit DCTCoefs(std::span<const size_t, 1> size):
      coefs({size[0], size[0]}) {
    const float c1 = std::sqrt(2.f / size[0]);
    const float c2 = 1.f / std::sqrt(2.f);
    const float c3 = 1.f / (2.f * size[0]);

    for (size_t u = 0; u < size[0]; ++u) {
      for (size_t x = 0; x < size[0]; ++x) {
        coefs[{x, u}] =
            std::cos(((2 * x + 1) * u * std::numbers::pi) * c3)
            * c1;
      }
    }
  ...
  [[nodiscard]] float operator[](
      const std::array<size_t, 2> &position) const {
...
public:
  ...
  explicit DCTCoefs(std::span<const size_t, D> size):
      current(size.template last<1>()),
