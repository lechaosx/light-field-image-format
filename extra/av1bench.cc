...
void print_usage(char *argv0) {
  ...
  std::cerr << argv0 << " -i <input-file-mask> -o <output-file-name> [-f <fist-bitrate>] [-l <last-bitrate>] [-a] [-n]" << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  char opt {};
  while ((opt = getopt(argc, argv, "hi:o:f:l:an")) >= 0) {
    switch (opt) {
      case 'h':
        print_usage(argv[0]);
        return 0;
  ...
  if (!decoder) {
    std::cerr << "decoder AV_CODEC_ID_AV1 not found" << std::endl;
    exit(1);
  }

  coder = avcodec_find_encoder(AV_CODEC_ID_AV1);
  if (!coder) {
    std::cerr << "coder AV_CODEC_ID_AV1 not found" << std::endl;
    exit(1);
  }
  ...
  if (append) {
    ...
    output << "'av1' 'PSNR [dB]' 'bitrate [bpp]'" << std::endl;
  }
  if (!output) {
